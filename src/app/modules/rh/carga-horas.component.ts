...
export class CargaHorasComponent {
  ...
  processarHoras() {
  this.loading = true;
  this.message = 'Processando horas...';

  this.http.post('http://localhost:3000/calcular/processar', {})
    .subscribe({
      next: (res: any) => {
        this.loading = false;
        this.message = 'Horas processadas com sucesso!';
      },
      error: (err) => {
        console.error(err);
        this.loading = false;
        this.message = 'Erro ao processar horas.';
      }
    });
}

exportarExcel() {
  this.http.get('http://localhost:3000/exportar/exportar-excel', { responseType: 'blob' })
    .subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'resumo_horas.xlsx';
        a.click();
        window.URL.revokeObjectURL(url);
      },
      error: (err) => {
        console.error(err);
        this.message = 'Erro ao exportar Excel.';
      }
    });
}
